...
public class DecompressorScreen extends AbstractContainerScreen<DecompressorMenu> {
    ...
    private static final int MIN_P = DecompressorBlockEntity.MIN_PRESSURE;
    private static final int MAX_P = DecompressorBlockEntity.MAX_PRESSURE;
    ...
    protected void extractLabels(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        // Status line
        HudWriter procWriter = new HudWriter(graphics, this.font, 52, 33, 10, false);
        float kfPct = menu.getKfProgressScaled();
        if (menu.getProcessCooldown() > 0) {
            procWriter.setColor(0xFF44AAFF).write("processing");
        } else if (kfPct > 0f) {
        ...
        HudWriter kfWriter = new HudWriter(graphics, this.font, 52, 63, 10, false);
