...
public class ReactorBER implements BlockEntityRenderer<ReactorBlockEntity, ReactorBERRenderState> {
    ...
    public void extractRenderState(ReactorBlockEntity entity, ReactorBERRenderState state,
        ...
        ReactorStructure structure = entity.getStructure();
        if (structure == null) return;

        BlockPos master = entity.getBlockPos();
        ClientLevel level = entity.getLevel() instanceof ClientLevel cl ? cl : null;

        List<BlockPos> cells = structure.cells;
        for (int i = 0; i < cells.size() && i < entity.getContainerSize(); i++) {
            ItemStack stack = entity.getItem(i);
            if (!(stack.getItem() instanceof ReactorControlRodItem)) continue;
            Integer control = stack.get(OmniTechDataComponents.ROD_CONTROL.get());
            if (control == null) continue;

            BlockPos cell = cells.get(i);
            int light = level != null
                    ? LevelRenderer.getLightCoords(level, cell.above())
                    : LightCoordsUtil.FULL_BRIGHT;

            state.rods.add(new ReactorBERRenderState.RodEntry(
                    cell.getX() - master.getX(),
                    cell.getY() - master.getY(),
                    cell.getZ() - master.getZ(),
                    control, light));
        }
    }
